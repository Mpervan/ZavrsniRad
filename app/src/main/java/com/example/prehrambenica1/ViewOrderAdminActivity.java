...
public class ViewOrderAdminActivity extends AppCompatActivity {
    ...
    private Button viewOrder, home, confirm, cancel;
    private DBHelper DB;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        confirm.setOnClickListener((new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DB.orderStatusChange(order.getNumber(), time, "In progress");
                Intent intent = new Intent(getApplicationContext(), ConfirmOrderAdminActivity.class);
                intent.putExtra("key_order", String.valueOf(order.getID()));
                startActivity(intent);
            }
        }));
        cancel.setOnClickListener((new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                DB.orderStatusChange(order.getNumber(), time, "CANCELED");
        ...
        if(order.getNumber() <= 9 ){
            num = "New order number: 00" + numRead;
            number.setText(num);
        }
        if(order.getNumber() >= 9 && order.getNumber() < 100){
            num = "New order number: 0" + numRead;
            number.setText(num);
        }
        if(order.getNumber() > 99){
            num = "New order number: " + numRead;
            number.setText(num);
        }
        ...
        for (int i = 0; i < intList.size(); i++) {
            ...
            if (cursor.moveToFirst()) {
                ...
                if(imageBytes != null){
                    ...
                    bitmap = BitmapFactory.decodeResource(ViewOrderAdminActivity.this.getResources(), R.drawable.menu_item);
                }
                item.setImage(bitmap);
                itemList.add(item);
            }
        }
        adapter = new ItemListAdapter(ViewOrderAdminActivity.this, itemList);
